...
sentence_templates_noun = [
    ...
    "I'm looking for something that smell of {accord} or {accord_sub}. A perfume for {gender} by the way. I like it having {longevity}, {sillage} and {price}. It's for {usage}"
]
...
for idx, row in df.iterrows():
    for _ in range(2000):  # tạo 1500 câu cho mỗi dòng 
        ...
        personality_base = row.get("personality", "elegant").lower()
        accord_candidates = [
            str(row.get("mainaccord1", "")).lower(),
            str(row.get("mainaccord2", "")).lower(),
            str(row.get("mainaccord3", "")).lower(),
            str(row.get("mainaccord4", "")).lower(),
            str(row.get("mainaccord5", "")).lower()
        ]

        
        # Lọc ra các giá trị không rỗng và không phải NaN
        accord_candidates = [a for a in accord_candidates if isinstance(a, str) and a.strip().lower() != "nan" and a.strip() != ""]
        accord_base = random.choice(accord_candidates) if accord_candidates else "floral"
